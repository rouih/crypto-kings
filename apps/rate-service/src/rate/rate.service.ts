...
export class RateService implements OnModuleInit {
  ...
  constructor(
    ...
    this.coingeckoApiUrl =
      this.configService.get<string>('COINGECKO_URI') ||
      'https://api.coingecko.com/api/v3/simple/price';
    this.coinGeckoIds =
      this.configService.get<string>('COINGECKO_IDS') || 'bitcoin';
    this.coinGeckoCurrencies =
      this.configService.get('COINGECKO_CURRENCIES') || 'usd';
  }
  ...
  private async fetchRates(): Promise<Record<string, Record<string, number>>> {
    try {
      ...
      this.errorHandlerService.handleInternalServerError(
        'Failed to fetch rates from CoinGecko',
      );
    }
  }
  ...
  private async updateRatesWithRetries(): Promise<void> {
    ...
    while (attempt < this.maxRetries) {
      try {
        ...
        attempt++;
        this.logger.warn(
          `Update rates attempt ${attempt} failed. Retrying in ${this.retryDelay / 1000}s...`,
        );
        if (attempt === this.maxRetries) {
          this.errorHandlerService.handleInternalServerError(
            'Failed to update rates after maximum retries',
          );
        }
  ...
  async getRate(crypto: string, currency?: string): Promise<number> {
    ...
    if (rate === undefined) {
      ...
      this.errorHandlerService.handleNotFound(
        `Rate for ${crypto} in ${currency} is not available`,
      );
    }
