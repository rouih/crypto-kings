import { FileIOError, InternalServerException, NotFoundException } from '../../error-handling/exceptions/custom-exceptions.exeption';
  ...
  async readFile<T>(filePath: string): Promise<T> {
    try {
      ...
      if (error.code === 'ENOENT') {
        throw new FileIOError(`File not found at ${filePath}`);
      }
